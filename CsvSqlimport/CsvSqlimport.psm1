...
<# 
	...
	.PARAMETER CSV
	Required. The location of the CSV file(s) to be imported. Multiple files are allowed, so long as they all support the same 
	...
	.PARAMETER Database
	Required. The name of the database where the CSV will be imported into.
	...
	contains data (skips first row if -FirstRowColumnNames). Datatypes used are: bigint, numeric, datetime and varchar(MAX). 
	...
	Imports the comma delimited housing.csv to the SQL "markets" database on a SQL Server named sql001.
	...
	PROCESS {
		...
		if ($table.length -eq 0) { 
			...
			$no = New-Object System.Management.Automation.Host.ChoiceDescription "&No", "Allows you to specify a table name."
		...
		} catch { throw "Could not create SQL command object." }
...
<# 
	...
	PROCESS {
		...
		if ($table.length -eq 0) { 
			...
			$question = "Would you like to use the automatically generated name: $table"
